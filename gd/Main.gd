...
export (PackedScene) var Mob = preload("res://tscn/Mob.tscn")
export (PackedScene) var Safe = preload("res://tscn/Safe.tscn")
export (PackedScene) var Life = preload("res://tscn/Life.tscn")
#signal
signal accelerate
signal show_flash
signal clear_enemy
...
func _ready():
	...
	$Player.connect("update_life_pic",$HUD,"update_life")
...
func spawn_life():
	...
	add_child(life)
	var screen_size = get_viewport_rect().size
	life.connect("life",$Player,"on_get_life")
	life.connect("add_life_score",self,"add_food_count")
	life.position = Vector2(rand_range(50,screen_size.x-50),rand_range(50,screen_size.y-50))
	$HUD.connect("start_game", life, "on_start_game")
	
func spawn_food():
	#life:30%
	#safe:70%
	var flag = rand_range(0,9)
	if flag > 2:
		self.spawn_safe()
	else:
		self.spawn_life()
